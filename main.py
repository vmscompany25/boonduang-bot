...
def build_fortune_flex(fortune_text, day_info):
    ...
    # ป้องกัน Cache ด้วย Timestamp
    liff_url = f"https://web-production-f666c.up.railway.app/liff?t={int(time.time())}"
        ...
        "footer": {
            ...
            "contents": [
                ...
                {"type": "box", "layout": "horizontal", "contents": [
                    ...
                    {"type": "button", "action": {"type": "uri", "label": "🔮 เปิดดวง", "uri": liff_url}, "style": "primary", "color": "#7B2FBE", "flex": 1, "margin": "sm", "height": "sm"}
                ]}
            ]
        }
    }
...
# API ใหม่สำหรับหน้า LIFF เรียกขอข้อมูล AI
@app.route("/api/get_liff_karma")
def api_get_liff_karma():
    day_info = get_day_info()
...
- เชื่อมกับธาตุ{day_info['element']}หรือธรรมชาติ
...
ตอบภาษาไทย ไม่ต้องมีหัวข้อ"""
    try:
        ...
        print(f"Error api_get_liff_karma: {e}")
        return jsonify({"text": "บุญดวงหมดแรง... แกไปทำดีเอาเองก่อนแล้วกันวันนี้ 😤"}), 500
